...
import { UserPlus, Phone, Lock, ArrowLeft, CheckCircle, Eye, EyeOff } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#2d5a4d] via-[#235043] to-[#1a4038] flex items-center justify-center p-6">
      <Card className="w-full max-w-md p-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="w-12 h-12 bg-[#2d5a4d] rounded-lg flex items-center justify-center">
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
          <div className={`w-3 h-3 rounded-full ${step >= 1 ? 'bg-[#2d5a4d]' : 'bg-gray-300'}`} />
          <div className={`w-16 h-1 ${step >= 2 ? 'bg-[#2d5a4d]' : 'bg-gray-300'}`} />
          <div className={`w-3 h-3 rounded-full ${step >= 2 ? 'bg-[#2d5a4d]' : 'bg-gray-300'}`} />
        </div>
        ...
        {step === 1 && (
          <form onSubmit={handleCheckPhone} className="space-y-4">
            ...
            <Button
              type="submit"
              className="w-full bg-[#2d5a4d] hover:bg-[#1a4038] text-white py-6"
              disabled={loading}
          ...
          <form onSubmit={handleActivateAccount} className="space-y-4">
            ...
            <div className="bg-green-50 p-4 rounded-lg border-l-4 border-green-600">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-semibold text-green-800">Profil trouvé!</span>
              </div>
              <div className="space-y-1 text-sm">
                <p><span className="text-gray-500">Nom:</span> <span className="font-medium">{memberInfo.member_name}</span></p>
                <p><span className="text-gray-500">Coopérative:</span> <span className="font-medium">{memberInfo.cooperative_name}</span></p>
                {memberInfo.village && (
                  <p><span className="text-gray-500">Village:</span> <span className="font-medium">{memberInfo.village}</span></p>
                )}
              </div>
            </div>
