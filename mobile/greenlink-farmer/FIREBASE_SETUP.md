...
TOKEN=$(curl -s -X POST "https://activation-flow-6.preview.emergentagent.com/api/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"identifier":"farmer1@test.com","password":"test123"}' | jq -r '.access_token')

# Envoyer une notification de test
curl -X POST "https://activation-flow-6.preview.emergentagent.com/api/greenlink/notifications/test-push" \
