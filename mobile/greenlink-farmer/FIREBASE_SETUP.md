...
TOKEN=$(curl -s -X POST "https://apk-build-test.preview.emergentagent.com/api/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"identifier":"farmer1@test.com","password":"test123"}' | jq -r '.access_token')

# Envoyer une notification de test
curl -X POST "https://apk-build-test.preview.emergentagent.com/api/greenlink/notifications/test-push" \
