...
const USSDCarbonScreen = () => {
  ...
  useEffect(() => {
    setSessionId(`mobile_carbon_${Date.now()}`);
  }, []);

  const startSession = async () => {
    const newSid = `mobile_carbon_${Date.now()}`;
    setSessionId(newSid);
    setTextHistory('');
    setResult(null);
    await sendUSSD('', newSid);
  };
  ...
  const sendUSSD = async (input, sid = null) => {
    ...
    try {
      ...
      const res = await api.post('/ussd/carbon-calculator', {
      ...
      {!sessionActive && !screen && (
        <View style={styles.howItWorks}>
          ...
          <View style={styles.howSteps}>
            ...
            <View style={styles.howStep}>
              <View style={styles.howIcon}>
                <Text style={{ fontWeight: 'bold', color: COLORS.primary, fontSize: 16 }}>8</Text>
              </View>
              <Text style={styles.howText}>8 questions{'\n'}simples</Text>
            </View>
            <View style={styles.howStep}>
              <View style={styles.howIcon}>
      ...
      >
        ...
        <View style={styles.ussdContainer}>
          <View style={styles.ussdHeader}>
            <Ionicons name="cellular" size={14} color="#666" />
            ...
            {!sessionActive && !screen ? (
              <View style={styles.welcomeContainer}>
                ...
                <Text style={styles.welcomeDesc}>
                  Repondez a 8 questions simples pour connaitre votre prime carbone estimee en FCFA/kg
                </Text>
